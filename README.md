...
> **Antigravity 支持预览**：`story-setup` 可把 22 个 Skills、7 个 custom agents、Always-On Rule 和 workspace Hooks 完整部署到项目 `.agents/`。部署器不修改 `~/.gemini/`，不依赖全局目录或 symlink；`.agents/hooks.json` 只替换顶层 `oh-story` 管理组并保留用户组。本契约使用 `agents_version: 30`，部署后需新开 Antigravity conversation；IDE 与交互式 `agy` 建议分别试用。
...
**OpenClaw 用户：** 当前支持 skills-only：OpenClaw 可从 workspace `skills/`、`.agents/skills`、`~/.agents/skills`、`~/.openclaw/skills` 等 skill root 发现本项目 22 个 skill；`SKILL.md` 已按 OpenClaw 要求使用单行 `name` / `description` 与单行 JSON `metadata.openclaw`。`story-setup` 选择 `target_cli=openclaw` 时会把 skills 复制到项目 `skills/` 并写入 OpenClaw 版 `AGENTS.md`；agents/hooks 暂不部署，写正文前大纲守卫在 OpenClaw 下是 skill 内软约束。部署后如未显示新 skills，请新开 OpenClaw session 或等待 skills watcher 刷新。

**Reasonix 用户：** 当前支持 skills + 原生 plugin manifest：Reasonix 原生扫描项目 skill root（`.agents/skills` 等，指向 `skills/` 的 symlink）发现 22 个 skill，用 `reasonix doctor capabilities` 校验；也可用根 `reasonix-plugin.json` 走 `reasonix plugin install`。`story-setup` 选择 `target_cli=reasonix` 时会把 skills 复制到项目 `skills/` 并写入 Reasonix 版 `AGENTS.md`；hooks/custom agents 暂不部署，涉及专业 Agent 的 skill 走 solo/direct fallback。Windows 未启用 symlink 时改走原生 plugin。
...
**OpenClaw / Reasonix / 通用路径的目录残留要手动清：** 这三条路径的 skill 副本在项目 `skills/` 里，重跑 `/story-setup` 执行的就是项目里那份，自动清理到不了。项目里若出现 `skills/story-setup/references/agent-references/agent-references/`（可能嵌了多层）或 `skills/story-setup/skills/`，手动删掉。要让项目里的 skill 文本本身更新，还需要重新安装本项目后，用新包覆盖项目 `skills/` 下这 22 个目录。
...
| `story-project-pipeline-monitor` | `/story dashboard` `/全流程监测` | 全流程检测 · 状态、失效、阻塞、Reader Debt 与 Skill Candidate 膨胀诊断 |
| `story-workflow` | `/story-workflow` `/全流程写作` | 全流程编排 · 冷读后强制衔接去 AI 味、中文终校与发布门禁 |
| `story-natural-drafting` | `/story-natural-drafting` `/自然起草` | 自然起草 · 读者现场感、角色有限认知、非模板化句法 |
| `story-prose-style` | `/story-prose-style` `/文风` | 文风控制 · 叙事距离、句式节奏、意象与角色化表达 |
| `story-reader-cold-read` | `/story-reader-cold-read` `/冷读` | 读者冷读 · 信息差、翻页动力、情绪兑现与连续性 |
| `story-chinese-proofreading` | `/story-chinese-proofreading` `/中文终校` | 中文终校 · 错别字、标点、语病与阅读心流 |
| `story-originality-audit` | `/story-originality-audit` `/原创性审计` | 原创审计 · 表达、桥段与结构近似风险 |
| `story-fanqie-compliance` | `/story-fanqie-compliance` `/番茄合规` | 平台合规 · 发布前内容、格式与规则检查 |
| `story-serial-performance-diagnostics` | `/story-serial-performance-diagnostics` `/连载诊断` | 连载诊断 · 留存、追读、兑现与章节表现 |
...
> 以上 22 个 Skill 都以本 fork 的 `skills/` 为唯一权威源；`.agents/skills`、`.claude/skills` 与 `.codex/skills` 只是部署副本，不应直接修改。`story-deslop` 的本地检查是写作 lint：blocking 只限确定性句式/标点问题，其他提示按读感判断；朱雀等外部检测只作自测参考，不替代人工读感。
