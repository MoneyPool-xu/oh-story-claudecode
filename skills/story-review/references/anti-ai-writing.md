...
不为句长设字数目标。句子长度由人物思考速度、场景压力、信息难度、叙事距离和作品声线共同决定。一个动作或冲击可以极短，完整的观察、推理或情绪链可以很长。

| 场景信号 | 节奏处理 |
|------|------|
| 冲击、决断、危险结果 | 可用短句压实，但不为「有力」连续切碎 |
| 观察、推理、动作链、情绪余波 | 可用中长句保持语义完整，但不把多个独立变化塞进一句 |
| 对话 | 长短随人物权力、目的和当下情绪，不按统计值校准 |
...
> 语料统计只用于写后发现极端异常，不反推为生成目标。样本均值是结果，不是好文字的原因。
...
- [ ] **情绪不重复解释**：上下文已成立就不加反应尾巴；需要落地时优先写选择、台词、策略和后果，直写最准时可以直写，身体反应不作默认替代
- [ ] **对话口语化**：无书面腔，不同角色语气可区分
- [ ] **标点不压平**：没有把质问、爆发、犹豫全部压成句号；也没有随机堆砌 `？`/`！`，或用 `……`/`——` 硬造停顿
- [ ] **Show / Tell 按功能选择**：关键证据、选择和后果让读者经历；过场、低强度感受和已知事实可简洁直写
- [ ] **句长服务场景**：没有连续碎成提纲，也没有连续堆成无法跟随的长句；长短变化来自压力、思考、声线和叙事距离，不追统计均值
