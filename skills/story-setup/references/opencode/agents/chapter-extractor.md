...
>     "causal_chain": ["<初始压力>", "<读者期待>", "<主角行动>", "<复杂化>", "<兑现或延迟>", "<新问题>"],
...
> 本节只解释该章因果如何成立，不作为下游 Writer 的结构模板；没有明显机制时如实标注过渡功能，不替作者强行总结技巧。
...
不设数量硬下限。短章通常 3-8 个、普通章通常 6-15 个、大章可更多，这些只是软参考。删除一个节点后，若风险、信息、关系、目标、资源、身份、能力、决策、行动方向或读者判断没有遗漏，该节点就应合并或删除。连续微动作若服务同一戏剧目的，只算一个节点。
...
3. 情节点是否严格按时序排列
4. 每个情节点是否造成可识别状态变化；是否已删除或合并不改变状态的微动作切片
...
12. 是否输出了 `章节运行机制` / `chapter_mechanism`，且明确它是解释本章因果与读者推进，不是给 Writer 套用的公式
...
- **只读**：不修改任何文件，只输出提取结果
- **不做机制归纳**：只描述本章事实和运行过程；Reader Contract、期待债与 Skill 规则留给 Stage 4-5，禁止反向污染事实提取
