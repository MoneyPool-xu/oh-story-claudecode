...
| 参考文件 | 必读条件 |
|---|---|
| `story-setup/references/agent-references/writing-craft.md` | **产出正文全程**（首稿/重写/补写，落笔前必读）：**从细纲到正文·不照抄形状**、人物行动、注意力锁、决策性精确、疏密分配 |
| `story-setup/references/agent-references/narrative-kernel.md` | **产出正文全程，落笔前必读**：人物目标、读者问题、POV、因果选择、信息时机与兑现；平台约束不得覆盖它 |
| `story-setup/references/agent-references/reader-seat-prose.md` | **产出、重写或去味任何正文时，落笔前必读**：建立读者席位、角色/读者知识差、禁止泄露、深度限知与写后读者回放 |
| `story-setup/references/agent-references/banned-words.md` | **初稿落盘后的去味/审稿阶段**（只处理无功能的异常聚集，不在初稿时反检测写作） |
...
2. **行动与注意力**：明确主视角眼前要办成什么、什么挡住他、读者此刻最想确认什么，以及本章不能提前泄露什么。卖点/情绪只用于写后校准，不用来配置正文元素。
...
4. **动态承接**：把读者当前问题和人物当前任务当作磁场，不当逐段 KPI。允许少量观察、走路、闲聊、等待和无回收的生活噪音，只要读者仍知道为什么继续读；只有无理由丢失注意力或连续停滞才删除/后移。
...
**叙述姿态（默认·深度限知）**：按 `reader-seat-prose.md` 锁定一个意识中心，让读者与主视角同步感知、判断和误判；不切他人内心，不提前剧透，不写中立摄像机。叙述者不长期比视角人物更聪明：人物只觉得不对，就允许正文停在不对。

**分层输入**：先执行 `narrative_kernel`，再执行题材卡；`platform_constraints` 只调整本章信息排序、卖点显现和兑现速度，不能指定固定字数爆点、统一短句、机械打脸或角色救场。发生冲突时保留人物因果、读者席位与自然表达，并在回执报告冲突。
...
- 物件复现只在本场有用途时使用；不追踪「第几次」，不为意义翻转强行点出它
...
- 开头尽快建立一个清晰的读者问题和当前赌注；不按前 100 字事件数验收
...
- 如有已经自然入场的物件可承担最后变化，可回扣；不为收尾硬拉道具回场
...
- **Gate A 套词聚集复核**：在初稿后检查命运齿轮/如潮水般/仿佛春风/心猛地一沉/眼眶泛红等是否成片复现。单处贴人物、贴语境且有功能可保留；不做同义词轮换（查 `story-setup/references/agent-references/banned-words.md`）
...
- **Gate D 节奏调整**：只拆臃肿修饰、堆叠比喻和无法跟随的信息过载，也不把完整的观察、推理或情绪链切成提纲。句长服从人物思考速度、场景压力、信息难度、叙事距离和作品声线，不追数值带
...
- **禁止连续停滞**：不把每句都写成任务；允许普通动作、环境噪音、闲话和无象征意义的生活细节。只在它们连续堆积、让读者失去当前问题时压缩
...
**生成与检测隔离**：初稿时不为了回避检测类别改变正常表达。正文落盘后再跑 `check-ai-patterns.js --check --fail-on=blocking <正文>` 与 `check-outline-copy.js <正文>`：只有破折号/格式与明确毒句式等 blocking 必须修；`advisory` 只定位异常聚集，逐条读语境，有叙事功能就保留，不追求归零。node 不可用时如实报告。
