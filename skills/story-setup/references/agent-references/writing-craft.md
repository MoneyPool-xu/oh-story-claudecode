...
> 以人物行动和读者注意力为核心的可执行指令。技法是候选工具，不是正文配额。

---

**本文件职责**（其他内容去哪看）：
- 本文件只管「有功能的物件复现」；羁绊功能按任务 profile 读取 long-emotional-methods.md 或 short-emotional-methods.md
- 对话相关（含权力博弈的规则/模式/示例/操作指令）一律见 dialogue-mastery.md，本文件不重复
- 本文件只管「开头注意力锁」，不管「开头设计」（开头设计见 opening-design.md）
- 长篇题材声线走 genre-prose-cards.md，短篇结构走 short-genre-formulas.md；本文件只提供通用正文方法
- 本文件第 8 节「场景写法（三维度揉进）」是 Phase 3 的写作方法，format-and-structure.md 负责结构规范
- 跨章复现和回收时机属于大纲/追踪层。正文 agent 只接收本场为何会注意该物件，不接收「第几次出现」或「必须翻转意义」。
...
物件只在角色会用、会注意，或它会改变信息、关系、选择和后果时出现。复现次数由故事需要决定；一次就完成功能的物件不必回收，需要反复使用的物件也不必每次改变象征意义。

### 大纲层备选编排

`1 → 2 → 3` 只是可选的规划模型，不是交付门槛。只有用户、大纲或题材明确需要信物回扣时才使用：
...
只在角色必须用数字做决定，或数字构成时限、交易、风险、证据、规则时保留精确值。数字不是专业感或情绪重量的默认修辞；角色不会量、记、算的数字，不由叙述者代为精确。

### 操作指令

| 场景 | 操作 | 示例 |
|------|------|------|
| 角色据此判断 | 保留决定所需的金额/年限 | 「还剩十一秒」会改变逃离选择 |
...
开头必须尽快建立一个清晰、具体、能继续发展的读者问题。一个强问题足够；不按字数统计事件，不为提高密度提前塞入身份、设定和后续反转。

### 操作指令

- 读者读完开头后能用一句话说出「我现在想知道什么」。
- 背景只给理解当前问题所需的部分；氛围型开头可以从环境入镜，但环境必须改变威胁、判断或期待。
- 新事实若不推进当前问题，后移到角色需要它的时候。
