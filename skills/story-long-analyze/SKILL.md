...
| 阶段 | 名称 | 输入 | 输出 | 完成标志 |
|------|------|------|------|----------|
| 0 | 原文与章节边界 | 原始文本 | 原文备份 + 概要.md（200 字 thin first-pass）+ `_progress.md` 章节边界 | 章节结构识别完成 + 章节边界落盘 |
...
`拆文报告.md` 出来后（Stage 6 跑完）执行。文风失败不影响回填。
...
### Stage 3 内部并行与 Stage 4-5 串行

**并行执行图**：
```
Stage 3a（剧情聚合 + 角色合并）       ──┐
                                        ├── 可并行
Stage 3b（设定：世界观/金手指/势力）  ──┘
              ▼
Stage 3c（角色完整档案）
              │
              ▼
Stage 3d（角色关系提取）
              ▼
Stage 4（读者动力机制）→ Stage 5（Skill 提炼）→ Stage 6（文风与总报告）
```

Stage 4/5 必须在事实层冻结后运行；不得把机制判断反写进 Stage 2 情节点。
...
2. **质量失败**（输出落盘后跑 chapter-extractor.md 自检，任一不达标——典型：把微动作拆成伪节点、P 行缺白描、概要条目化、枚举越界、角色名为昵称/通用称呼）→ **升级到 sonnet 重试 1 次**
...
Stage 3-5 分块见 [material-decomposition.md](references/material-decomposition.md)；Stage 4-5 的分析与分类契约见 [narrative-mechanism-extraction.md](references/narrative-mechanism-extraction.md)。
