...
## 拆解流程：事实提取 → 聚合 → 机制解释 → Skill 提炼

本文件主要定义 Stage 0-3 的事实层方法。Stage 4-5 的机制解释与规则提炼以 `narrative-mechanism-extraction.md` 为准，Stage 6 负责文风与总报告；不在这里维护另一套阶段编号。
...
#### B. 情节点提取（状态变化单位，不设硬下限）

数量由有效状态变化决定，字数只用于防止明显漏拆，不用于凑配额。

| 字数 | 目标范围 | 说明 |
|------|---------|------|
| ≤1000 | 通常 3-8 个 | 只保留有效状态变化，不拆微动作凑数 |
| 3000 | 通常 6-15 个 | 普通章软参考 |
| 5000 | 可多于 15 个 | 按场景与变化决定 |
| 8000+ | 可继续增加 | 不遗漏关键变化，也不拆微动作 |

删除测试：去掉节点后，风险、信息、关系、目标、资源、身份、能力、决策、行动方向或读者判断若均无遗漏，就合并或删除该节点。
...
标准章节额外输出 `关键信息与扩写技法` 表，作为后续 `剧情/节奏.md` 和 `剧情/情绪模块.md` 的基础数据。它避免主观点评，负责把“本章必须传递的剧情信息”与“作者如何把它扩写成可读场景”拆开。
...
质量要求：有明显扩写机制时如实提取，通常 1-5 条，不设每章最低数量。纯移动、时间跨越、行政手续、生活恢复或事实搬运章没有值得独立抽象的机制时，明确写「本章主要承担过渡/事实搬运，无值得独立抽象的扩写机制」，不得替作者强行合理化。
...
> **语料读取**：剧情聚合、设定、关系和散落情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md` **一次性读取并复用**，不逐文件重复扫描。汇总文件缺失或语料超上下文时，按下方分块策略处理。
...
落在经验区间外时，只检查是否存在过细、过粗、漏线或重复归类；检查通过就保留真实数量，不得为了回到区间强拆或强并。首要原则始终是核心戏剧目标与叙事弧完整。
...
**框架识别后自检**（聚合前必须通过）：
1. 覆盖率诊断：记录所有剧情覆盖的章节比例；<85% 时检查漏归类，>95% 时只复核边界，数值本身不判失败
2. 粒度检查：是否存在过细（单一事件）或过粗（宏观主题）的剧情，执行粒度层级混乱检测
3. 独立性检查：每条剧情是否满足 4 项独立性标准
4. 数量诊断：若超出经验区间，是否已经复核粒度；超出本身不判失败
...
**三层覆盖策略**：
1. **主线剧情**（优先级最高）：推动整体故事发展，常见样本约占 60-70% 章节
2. **支线剧情**：感情线、成长线、战斗线等，常见样本约占 20-30% 章节
3. **补充剧情**：识别确有独立功能的过渡/日常剧情；不得为了提高覆盖率虚构剧情单元

上述比例只用于描述和发现明显漏线，不是归类目标。真实结构成立时接受任何比例，不得为贴近经验值调整剧情归属。
...
#### D. 小说概要生成（在 Stage 6 落盘，覆盖 Stage 0 thin first-pass）

整体故事框架识别 + 500-1000字高密度全书概要（涵盖主要剧情线阶段性发展、核心人物作用、关键转折点、因果关系）。

> **落盘时机**：Stage 3 产出概要素材，Stage 6 总报告时写入 `概要.md`，覆盖 Stage 0 thin first-pass。
...
> 模板与示例见 [output-templates.md](output-templates.md) 「Stage 3 设定+关系」节。
...
Stage 3-6 都是派生产物，但证据要求分两类：

- **硬事实**必须能回到原文或已核验的 Stage 2 事实节点。
- **机制判断**不要求原文逐字出现，但必须回到 Stage 2/3 的具体章节、情节点或剧情单元；跨章机制至少标出建立端与支付/变化端。

离原文越远，越容易把「摘要没说的」补成坐实事实，或把印象写成无证据评论。三条硬约束：
...
落盘后做一次**双轨证据自检**：硬事实抽查能否回到原文；机制判断检查能否回到 Stage 2/3 的明确证据引用。前者命中不到就改为「原文未明确」或删除，后者无定位就标「待验证」或删除。
...
Stage 3 事实聚合完成后自检。下列数值都是异常诊断器，不是优化目标：

| 指标 | 阈值 | 计算 | 不达标处理 |
|------|------|------|------------|
| 置信度 | >= 0.85 | 有明确归属的情节点 / 剧情单元内情节点总数 | 低于 0.85 标记「待复核」 |
| 覆盖率 | 诊断参考 | 已归类情节点 / 总情节点数 | <85% 检查漏归类；>95% 仅提示复核边界，不判失败、不为降数而拆分 |
| 关键信息覆盖 | 诊断参考 | 有机制条目或明确「无独立机制」的章节数 / 摘要章节数 | 缺少说明才回 Stage 2；诚实的「无独立机制」视为已检查 |
...
6. **覆盖率验证**：
   - 覆盖率 = (总数 - 散落数) / 总数 × 100%
   - < 85% → 检查是否漏归类；只对确有归属的节点重试
   - > 95% → 仅检查是否过度归并；检查通过即接受，不人为制造散落项
