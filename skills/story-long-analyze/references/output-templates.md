...
本文件按唯一管道组织：Stage 0-3 事实层、Stage 4 机制层、Stage 5 提炼层、Stage 6 文风与总报告。
...
> **范围说明**：Stage 0 只能基于章节标题、卷段结构和少量抽样产出 thin first-pass（~200 字）。完整版在 Stage 6 落盘覆盖。
...
> Stage 6 完成后会就地覆盖「首版概要」为全书概要（500-1000字 plot-aware），章节索引和卷段表保留。
...
**情节点**（以可识别的状态变化为单位，不设数量硬下限；短章通常 3-8、普通章通常 6-15，仅作软参考）：

删除该节点后，若风险、信息、关系、目标、资源、身份、能力、决策、行动方向或读者判断均无遗漏，就不独立成节点。连续微动作服务同一变化时合并。
...
**Stage 2 输出自检**：概要连贯客观｜节点均造成可识别状态变化，微动作已合并｜白描写全事实、结果和原文给出的起因｜严格时序｜精选引用不超过 8 条｜角色用全名｜枚举合法｜不跨章｜`关键信息与扩写技法` 与 `章节运行机制` 齐全。Stage 2 禁止填写 Reader Contract、期待债或可迁移规则。
...
| 判断依据 | {哪些章节/情节点支撑此判断} |
| 剧情数量诊断 | {实际数量；若超出经验区间，记录粒度复核结果，不要求回到区间} |
```
...
6. 覆盖率诊断：参考 [material-decomposition.md 质量诊断体系](material-decomposition.md)，异常时检查漏归或过度归并，不为命中区间而改数据

### Stage 3 覆盖率

覆盖率 = 已归类数 / 总数 x 100%，解释方式见 [material-decomposition.md 质量诊断体系](material-decomposition.md)；它是诊断值，不是目标函数。

## Stage 3 设定+关系（3b/3c/3d）

> **Stage 4 直接按主题拆分输出多个文件**（与当前导入、长篇写作项目结构对齐）。下游不再现场拆分扁平文件。
> **证据保真**：硬事实回原文核对；机制判断回指 Stage 2/3 证据。无来源分别写「原文未明确」或「待验证」（见 material-decomposition.md「所有派生阶段的证据保真」）。
...
按 [narrative-mechanism-extraction.md](narrative-mechanism-extraction.md) 输出 `读者契约.md`、`期待债.md`、`情绪债.md`、`关系债.md`、`成长与升级.md`。每条判断必须带来源；优先复用 Stage 2-3 产物，不重读整本原文。
...
- **读者契约与长线阅读动力**：摘要 Stage 4 五个机制文件，展示许诺、压力、延迟、部分支付、兑现和新问题
- **结构分析**：主线 / 副线 / 剧情单元 / 覆盖率
- **关键信息与扩写技法总览**：按章节/剧情单元汇总关键剧情信息如何被扩写（详情以 `章节/*_摘要.md` 与 `剧情/节奏.md` 为准）
- **全书情绪节奏总览**：摘要 `剧情/节奏.md` 的情绪折线、爽点频率、小/中/大高潮分布、循环单元
- **兑现结构**：频率只作观察；权威结论写 Promise → Pressure → Delay → Partial Payment → Payoff → Aftermath
- **节奏与情绪触动点**：摘要 `剧情/节奏.md` 的爆发密度、触动点分布、长间隔风险、爆发节奏
- **伏笔与冲突网络**：跨章伏笔地图（仅列跨度超过2章或影响主线的伏笔）+ 冲突升级路径（低级→中级→高级→终局）
- **核心机制**：全书最强 5 个读者动力机制及证据
- **角色体系**：主角/反派/核心配角/功能角色分级概览
- **质量评估**：置信度 / 覆盖率 / 重叠率
- **系统诊断**：只引用 `_diagnostics.json` 中的 Stage 4 Reader Debt Inflation 与 Stage 5 Skill Candidate Inflation 派生摘要，列出诊断等级、复核对象和证据缺口；不在报告里另算一套指标
- **Skill 提炼结论**：分别摘要 Core / Genre / Author / Risky / Anti-pattern，不把单书观察伪装成通用规律
