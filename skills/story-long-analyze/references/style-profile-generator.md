...
**关键格式注意**：`章节/*_摘要.md` 的实际格式是 `主题标签X | 基调：Y` 单独成行、跟在每个情节点后（节点数随有效状态变化而定，不设硬下限）。`基调` 用**全角冒号**，`主题标签` 后**不带冒号**，两者都**不在行首**。grep 模式不能用 `^基调:` 这种锚定。
...
| Stage 4-5 机制/提炼产物不存在 | **停止总报告合成**，提示先跑完 Stage 4-5；文风抽样可降级生成并注明缺失 |
