...
> 质量诊断（置信度 / 覆盖率 / 重叠率）见 [material-decomposition.md 质量诊断体系](material-decomposition.md)。统计用于发现异常，不作为目标函数。
...
存量拆文若 Stage 2-3 事实产物齐全，可直接从 Stage 4 补跑机制与提炼；不得伪造缺失事实，也不要求重跑 Stage 0-3。旧阶段编号按实际产物识别，不只按数字猜测。
