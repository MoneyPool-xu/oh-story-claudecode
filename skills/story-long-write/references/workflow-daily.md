...
4. **三章校准点**：当最后提交章号为 3 的倍数时，按 `character-plot-calibration.md` 从当前 state 修订号、近三章增量和必要的历史定点证据更新 `报告/连续性/角色与剧情台账.md`，检查冲突换皮重复、已兑现战果复用、底牌遗忘、角色能力/性格越界和关系阶段倒退。它是带版本号的派生诊断，不得成为第二权威状态；下一章只加载相关小节，不注入整份历史。
