...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - 目标平台为七猫/无线买量，或项目明确启用章节商业门禁时，先读取 `chapter-commercial-gate.md`，对当前细纲完成六维对账并写入 `audit_logs/chapter_{N}_gate.json`。细纲哈希、组合门禁或 `decision` 不通过时先修细纲，不调 narrative-writer；金手指不适用、没有实物收益本身不构成失败。
