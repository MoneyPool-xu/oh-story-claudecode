...
- 目标平台为番茄短篇 → 加载 `platforms/fanqie-short-story.md`；只应用平台风险层，不把经验数字当铁律 ✅/❌
...
4. **编写 小节大纲.md**：使用固定 12 列 Markdown 表格，每节 1 行，表头如下（列名和顺序不可改）：
   ...
   - 番茄短篇对每一行追加场景价值复读：当前欲望、主要阻碍、主动选择、状态变化、读者更新、承接/结清。答案可分布在现有 12 列中，不扩成重表；删除该节后各项均无损时先合并或重写，不得进入正文
...
### 开头
- [ ] 开篇是否尽早建立具体困境、异常或读者等待，而非为了“前三句”硬塞爆点？
- [ ] 没有无意义的背景介绍？
- [ ] 包含一个开篇钩子？
