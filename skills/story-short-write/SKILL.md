...
4. 目标平台为番茄短篇时，再完整读取 `references/platforms/fanqie-short-story.md`；该文件是平台风险层，不覆盖通用叙事内核
...
4. **开头尽早建立具体等待**。开篇要让读者知道人物正面对什么、为什么现在必须处理；“前三句”只作跳出风险复读点，不是句数门槛。
5. **视角服从亲密度与信息控制**。第一人称适合强主观和有限认知，第三人称限知适合需要距离、并行场景或更灵活的信息差；不得因平台标签一律选「我」。
...
4. 编写 小节大纲.md（格式见 writing-workflow.md）：短篇只做轻量蓝图，每节包含结构段/五段功能、人物/关系或其他状态变化、因果/逻辑链、结尾承接/钩子，不套长篇完整章节蓝图。**标出付费点卡在哪一节末**（见 `submission-craft.md`「付费点」：用未完成动作、身份/证据变化或两难选择形成真实断点）；用反推法先想透付费点那一节，再倒排前后。每节可选一个任务卡点，但必须服务情绪升级、证据推进、关系撕裂、反转铺垫或反击动作；没有就不强补
   - 番茄短篇逐节应用 `platforms/fanqie-short-story.md` 的场景价值门禁；门禁检查因果价值，不强制每节具备金手指、实物收益、打脸或悬崖钩子
...
- **步骤 3：自动生成逐节指令卡**：调用 `story-natural-drafting` 的 `references/short-scene-directive-template.md`，先建立短篇总卡，再为当前节生成 `报告/自然成稿/第NN节_写作指令.md`。卡与当前设定、大纲、平台和 POV 绑定；缺卡、旧卡或场景价值为 `BLOCK` 时不得起正文。
...
- 叙述必须留在已选 POV 的认知边界内：第一人称可以带主观审判，第三人称限知也必须带视角人物的注意偏向；只删越界的中立作者解说，不删人物真正会形成的判断。
...
- 小节用数字分割；每节必须造成可辨认的状态变化或完成必要的因果承接，不按情节点数量验收
- 情绪允许回落和换挡；若连续小节既无压力变化、局部支付、关系变化，也不更新读者判断，才判为空转
...
- 末节必须结清本篇核心读者契约；可以有余韵或后续空间，但不强制制造未完成悬念
