...
进入 Phase 3 正文或任何正文重写前，必须完整读取 `references/reader-seat-prose.md`，建立主视角、角色所知/读者所知、禁止泄露和情绪 A→B。短篇第一人称允许带「我」的主观审判，但不允许滑向中立作者解说或他人内心。
...
1. **先定人物眼前要解决的问题**。动笔前确定主视角想办成什么、什么挡住他、读者此刻最想确认什么。目标情绪是预期结果，不是反向配置脚步、旧物、心跳和灯光的任务。
2. **一个核心支点撑一篇**。反转型围绕一次主揭示蓄力；无反转型围绕报应兑现或甜度递进积累期待。不多线、不铺世界观。
3. **不让低功能内容连续停滞**。允许少量普通动作、闲话、环境噪音和无回收的生活细节；只在它们连续堆积、让读者丢失当前问题时删减。
...
**写前准备**（每个场景写前执行 2 步：锁定人物行动和读者注意力 → 只召回必要技法）：
- **步骤 1：行动+注意力锁**：① 主视角眼前要办成什么？② 读者此刻最想确认什么？③ 本场景的阻力会逼出什么选择或后果？答不出就回到大纲，不用情绪词、氛围或技法填空。如有 `对标/` 或 `拆文库/` 产出，只召回与当前行动问题有关的因果/信息差/声线手法，不把模块形状带进正文。
  - **多对标书时**：参 `references/cross-book-recall.md`，副对标/参考对标按阶段预算进入"副对标召回摘要"；正文只传摘要，不传副书文风或原文
- **步骤 2：必要技法确认**：只写一句内部意图：「人物为了 {A} 遇到 {B}，被迫 {C}，读者的问题从 {Q1} 变为 {Q2}。」技法只有在它帮助完成这条因果时才使用；情绪是预期结果，不是元素配置表。
...
**先写导语**：正文开头前先按 `references/submission-craft.md`「导语」写一条 150-220 字导语。导语只建立一个足够强的当前问题和必要赌注，不为满足四维骨架、黄金三角或事件数同时塞入身份、背景、道具和反转。它就是正文开头，写好顺势往下接、不重写。

**技法指令**：开头读完后，读者能用一句话说出「我现在最想知道什么」。不按前 100 字的事件数验收。
...
- 埋入误导信息，让读者猜错反转方向
- **决策性精确**：数字只在它改变时限、交易、风险、证据或角色选择时精确；不为专业感、情绪重量或递增形状额外添加数字
...
- 物件若承担反转证据，让它经由角色的发现或使用改变读者判断；不为意义颠覆硬做复现
...
- 有自然入场且能承担最后变化的物件时可回扣；不为满足「第三次」硬拉道具回场
